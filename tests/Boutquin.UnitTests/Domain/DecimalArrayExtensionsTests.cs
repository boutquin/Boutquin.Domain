...
#pragma warning restore CS8604 // Possible null reference argument.
    ...
    [Fact]
    public void Variance_TwoElementSample_DividesByNMinusOne()
    {
        // mean = 2; sum of squared deviations = 1 + 1 = 2; sample variance = 2 / (2 - 1) = 2.
        new[] { 1m, 3m }.Variance(CalculationType.Sample).Should().Be(2m);
    }

    [Fact]
    public void Variance_SingleElementPopulation_ReturnsZero()
    {
        // A one-element population has zero dispersion (the n>=2 rule is Sample-only).
        new[] { 5m }.Variance(CalculationType.Population).Should().Be(0m);
    }

    [Fact]
    public void StandardDeviation_SingleElementPopulation_ReturnsZero()
    {
        new[] { 5m }.StandardDeviation(CalculationType.Population).Should().Be(0m);
    }

    [Theory]
    [InlineData((CalculationType)(-1))]
    [InlineData((CalculationType)2)]
    public void AllMethods_ShouldRejectUndefinedCalculationType(CalculationType calculationType)
    {
        var values = new[] { 1m, 3m };

        Action variance = () => _ = values.Variance(calculationType);
        Action standardDeviation = () => _ = values.StandardDeviation(calculationType);

        variance.Should().Throw<ArgumentOutOfRangeException>()
            .WithParameterName(nameof(calculationType));
        standardDeviation.Should().Throw<ArgumentOutOfRangeException>()
            .WithParameterName(nameof(calculationType));
    }

    [Fact]
    public void AllMethods_NullInputWithUndefinedCalculationType_ThrowEmptyOrNullArrayException()
    {
        decimal[]? values = null;
        var calculationType = (CalculationType)(-1);
        Action[] operations =
        [
            () => _ = values!.Variance(calculationType),
            () => _ = values!.StandardDeviation(calculationType),
        ];

        foreach (var operation in operations)
        {
            operation.Should().Throw<EmptyOrNullArrayException>();
        }
    }

    [Fact]
    public void AllMethods_EmptyInputWithUndefinedCalculationType_ThrowEmptyOrNullArrayException()
    {
        var values = Array.Empty<decimal>();
        var calculationType = (CalculationType)(-1);
        Action[] operations =
    ...
    [Fact]
    public void AllMethods_SingletonInputWithUndefinedCalculationType_ThrowArgumentOutOfRange()
    {
        var values = new[] { 1m };
        var calculationType = (CalculationType)(-1);
        Action[] operations =
