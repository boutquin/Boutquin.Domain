...
/// - AgainstEmptyOrNullArray: Tests for null or empty arrays.
...
public sealed class GuardTests
{
    ...
    /// <summary>
    /// Tests that the Guard.AgainstEmptyOrNullArray method does not throw an exception when the array is not null or empty.
    /// </summary>
    [Fact]
    public void AgainstEmptyOrNullArray_WhenArrayIsNotNullOrEmpty_DoesNotThrow()
    {
        // Arrange
        var nonEmptyArray = new[] { 1, 2, 3 };

        // Act
        var act = () => Guard.AgainstEmptyOrNullArray(() => nonEmptyArray);
    ...
    /// <summary>
    /// Tests that the Guard.AgainstEmptyOrNullArray method throws an ArgumentException when the array is null.
    /// </summary>
    [Fact]
    public void AgainstEmptyOrNullArray_WhenArrayIsNull_ThrowsArgumentException()
    {
        // Arrange
        int[]? nullArray = null;

        // Act
        var act = () => Guard.AgainstEmptyOrNullArray(() => nullArray!);
    ...
    /// <summary>
    /// Tests that the Guard.AgainstEmptyOrNullArray method throws an ArgumentException when the array is empty.
    /// </summary>
    [Fact]
    public void AgainstEmptyOrNullArray_WhenArrayIsEmpty_ThrowsArgumentException()
    {
        // Arrange
        var emptyArray = Array.Empty<int>();

        // Act
        var act = () => Guard.AgainstEmptyOrNullArray(() => emptyArray);
    ...
    /// <summary>
    /// AC-4.2: <c>maxLength &lt;= 0</c> is itself a Guard-logic violation — checked via
    /// <c>AgainstNegativeOrZero</c> before the string is even inspected.
    /// </summary>
    [Theory]
    [InlineData(0)]
    [InlineData(-1)]
    public void AgainstOverflow_WhenMaxLengthIsZeroOrNegative_ThrowsArgumentOutOfRangeException(int maxLength)
    {
        var value = "short";

        Assert.Throws<ArgumentOutOfRangeException>(() => Guard.AgainstOverflow(() => value, maxLength));
    }
    ...
    [InlineData(0)]
    [InlineData(-1)]
    public void AgainstNullOrEmptyAndOverflow_WhenMaxLengthIsZeroOrNegative_ThrowsArgumentOutOfRangeException(int maxLength)
    {
        var value = "test";

        Assert.Throws<ArgumentOutOfRangeException>(() => Guard.AgainstNullOrEmptyAndOverflow(() => value, maxLength));
    }
    ...
    [InlineData(0)]
    [InlineData(-1)]
    public void AgainstNullOrWhiteSpaceAndOverflow_WhenMaxLengthIsZeroOrNegative_ThrowsArgumentOutOfRangeException(int maxLength)
    {
        var value = "test";

        Assert.Throws<ArgumentOutOfRangeException>(() => Guard.AgainstNullOrWhiteSpaceAndOverflow(() => value, maxLength));
    }
    ...
    /// <summary>
    /// Tests that the Guard.AgainstEmptyOrNullArray method throws an EmptyOrNullArrayException when the array is null.
    /// </summary>
    [Fact]
    public void AgainstEmptyOrNullArray_WhenArrayIsNull_ThrowsEmptyOrNullArrayException()
    {
        int[]? nullArray = null;

        Assert.Throws<EmptyOrNullArrayException>(() => Guard.AgainstEmptyOrNullArray(() => nullArray!));
    }

    /// <summary>
    /// Tests that the Guard.AgainstEmptyOrNullArray method does not throw an exception when the array is not empty.
    /// </summary>
    [Fact]
    public void AgainstEmptyOrNullArray_WhenArrayIsNotEmpty_DoesNotThrow()
    {
        var notEmptyArray = new int[1];

        var exception = Record.Exception(() => Guard.AgainstEmptyOrNullArray(() => notEmptyArray));
    ...
    public void WithArgs_WhenConditionIsTrue_ThrowsSpecifiedExceptionWithArgs()
    {
        ...
        var exception = Assert.Throws<ArgumentException>(() => guardCondition.WithArgs<ArgumentException>(["arg1", "arg2"]));
    ...
    public void WithArgs_WhenConditionIsFalse_DoesNotThrow()
    {
        ...
        var exception = Record.Exception(() => guardCondition.WithArgs<ArgumentException>(["arg1", "arg2"]));

        Assert.Null(exception);
    }

    /// <summary>
    /// Tests that AgainstOverflow rejects a null string with a clear ArgumentException rather than a
    /// NullReferenceException from the length check.
    /// </summary>
    [Fact]
    public void AgainstOverflow_WhenStringIsNull_ThrowsArgumentException()
    {
        string? value = null;

        var act = () => Guard.AgainstOverflow(() => value!, 10);
    ...
    /// <summary>
    /// Tests that AgainstEmptyOrNullEnumerable names the offending parameter in both the message and
    /// the ParamName property, consistent with the other collection guards.
    /// </summary>
    [Fact]
    public void AgainstEmptyOrNullEnumerable_WhenEmpty_MessageNamesParameter()
    {
        var emptyEnumerable = Enumerable.Empty<int>();

        var act = () => Guard.AgainstEmptyOrNullEnumerable(() => emptyEnumerable);

        act.Should().Throw<ArgumentException>()
            .WithParameterName("emptyEnumerable")
            .WithMessage("*emptyEnumerable*");
    }

    /// <summary>
    /// Tests that a null expression argument surfaces as InvalidOperationException from ExtractParameterInfo.
    /// </summary>
    [Fact]
    public void AgainstNull_WithNullExpression_ThrowsInvalidOperationException()
    {
        var act = () => Guard.AgainstNull((System.Linq.Expressions.Expression<Func<string>>)null!);
    ...
    /// <summary>
    /// Tests that a non-member-access expression body surfaces as InvalidOperationException — the
    /// expression family requires a direct member access (e.g. <c>() =&gt; field</c>), not a literal
    /// or a method call.
    /// </summary>
    [Fact]
    public void AgainstNull_WithNonMemberExpression_ThrowsInvalidOperationException()
    {
        var act = () => Guard.AgainstNull(() => "literal");

        act.Should().Throw<InvalidOperationException>();
    }

    /// <summary>
    /// Tests that the expression-based AgainstOutOfRange accepts the inclusive boundary values.
    /// </summary>
    [Fact]
    public void AgainstOutOfRange_Expression_AtInclusiveBoundaries_DoesNotThrow()
    {
        var min = 1;
        var max = 10;
        var atMin = () => Guard.AgainstOutOfRange(() => min, 1, 10);
        var atMax = () => Guard.AgainstOutOfRange(() => max, 1, 10);

        atMin.Should().NotThrow();
        atMax.Should().NotThrow();
    }

    /// <summary>
    /// Tests that the expression-based AgainstNegative accepts zero (the inclusive boundary).
    /// </summary>
    [Fact]
    public void AgainstNegative_Expression_AtZero_DoesNotThrow()
    {
        var zero = 0;
        var act = () => Guard.AgainstNegative(() => zero);

        act.Should().NotThrow();
    }

    /// <summary>
    /// G-08/G-10 / DE-3 / AC-2.8: the three empty/null container exceptions are re-based on
    /// <see cref="ArgumentException"/> so the guard family unifies under argument-exception catch
    /// semantics and the Guard XML docs become true.
    /// </summary>
    [Fact]
    public void EmptyOrNullExceptions_AreArgumentExceptionSubtypes()
    {
        typeof(EmptyOrNullArrayException).Should().BeAssignableTo<ArgumentException>();
        typeof(EmptyOrNullCollectionException).Should().BeAssignableTo<ArgumentException>();
        typeof(EmptyOrNullDictionaryException).Should().BeAssignableTo<ArgumentException>();
    }

    /// <summary>
    /// A single <c>catch (ArgumentException)</c> must catch every empty/null guard failure across the
    /// array, collection, dictionary, and enumerable guards.
    /// </summary>
    [Fact]
    public void EmptyOrNullGuards_AllThrowArgumentExceptionCompatible()
    {
        int[]? array = null;
        ...
        ((Action)(() => Guard.AgainstEmptyOrNullArray(() => array!)))
            .Should().Throw<ArgumentException>();
        ((Action)(() => Guard.AgainstEmptyOrNullCollection(() => collection!)))
            .Should().Throw<ArgumentException>();
        ((Action)(() => Guard.AgainstEmptyOrNullDictionary(() => dictionary!)))
            .Should().Throw<ArgumentException>();
        ((Action)(() => Guard.AgainstEmptyOrNullEnumerable(() => enumerable!)))
            .Should().Throw<ArgumentException>();
    }
}
