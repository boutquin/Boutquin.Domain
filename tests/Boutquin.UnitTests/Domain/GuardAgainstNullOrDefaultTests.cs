...
public class GuardAgainstNullOrDefaultTests
{
    ...
    /// <summary>
    /// G-07 / DE-4 / AC-2.7: for a <see cref="Nullable{T}"/>, a present-but-underlying-default value
    /// such as <c>(int?)0</c> must be rejected as the default value (an ArgumentException), because it
    /// carries no meaningful value — previously the default branch was unreachable and it passed.
    /// </summary>
    [Fact]
    public void AgainstNullOrDefault_NullableIntZero_ThrowsArgumentException()
    {
        int? value = 0;
        Assert.Throws<ArgumentException>(() => Guard.AgainstNullOrDefault(() => value));
    }
    ...
    /// <summary>
    /// A null <see cref="Nullable{T}"/> still surfaces as <see cref="ArgumentNullException"/>.
    /// </summary>
    [Fact]
    public void AgainstNullOrDefault_NullableIntNull_ThrowsArgumentNullException()
    {
        int? value = null;
        Assert.Throws<ArgumentNullException>(() => Guard.AgainstNullOrDefault(() => value));
    }
    ...
    /// <summary>
    /// A non-default <see cref="Nullable{T}"/> value passes.
    /// </summary>
    [Fact]
    public void AgainstNullOrDefault_NullableIntNonDefault_DoesNotThrow()
    {
        int? value = 1;
        Guard.AgainstNullOrDefault(() => value);
    }
    ...
    [Fact]
    public void AgainstNullOrDefault_NullableStructWithConstructor_Default_ThrowsArgumentException()
    {
        ConstructorInitializedValue? value = default(ConstructorInitializedValue);

        var exception = Assert.Throws<ArgumentException>(() => Guard.AgainstNullOrDefault(() => value));

        Assert.Equal(nameof(value), exception.ParamName);
    }

    [Fact]
    public void AgainstNullOrDefault_NullableStructWithConstructor_ConstructedValue_DoesNotThrow()
    {
        ConstructorInitializedValue? value = new ConstructorInitializedValue();

        var exception = Record.Exception(() => Guard.AgainstNullOrDefault(() => value));

        Assert.Null(exception);
    }

    private readonly record struct ConstructorInitializedValue(int Value)
    {
        public ConstructorInitializedValue() : this(42)
        {
        }
    }
}
