...
/// <summary>
/// A second Guid-backed identifier, used to verify that two distinct id subtypes wrapping the same
/// underlying value are not equal.
/// </summary>
public record OtherGuidId(Guid Value) : StronglyTypedId<Guid>(Value);
...
public sealed class StronglyTypedIdTests
{
    ...
    [Fact]
    public void DifferentSubtypes_SameUnderlyingValue_AreNotEqual()
    {
        // The core type-safety property: two distinct id types wrapping the same Guid must never be
        // equal (record equality discriminates by EqualityContract / runtime type).
        var guid = Guid.NewGuid();
        var first = new TestId(guid);
        var second = new OtherGuidId(guid);

        first.Equals(second).Should().BeFalse();
        ((object)first).Equals(second).Should().BeFalse();
    }

    [Fact]
    public void SameSubtype_EqualValue_HaveEqualHashCodes()
    {
        var guid = Guid.NewGuid();

        new TestId(guid).GetHashCode().Should().Be(new TestId(guid).GetHashCode());
    }
}
