...
/// <summary>
/// Contains unit tests for the <see cref="Boutquin.AspNetCore.Extensions.ModuleExtensions"/> class.
/// </summary>
/// <remarks>
/// <para>
...
public sealed class ModuleExtensionsTests
{
    ...
    /// <summary>
    /// A concrete <see cref="IModule"/> with no public parameterless constructor.
    /// Module discovery must skip it rather than letting <c>Activator.CreateInstance</c>
    /// throw <c>MissingMethodException</c> and abort the whole scan.
    /// </summary>
    public sealed class CtorlessModule : IModule
    {
        public CtorlessModule(string required) => _ = required;
    ...
    /// <summary>
    /// Verifies that <see cref="Boutquin.AspNetCore.Extensions.ModuleExtensions.MapEndpoints(WebApplication)"/> retrieves the registered
    ...
    /// </remarks>
    /// <summary>
    /// Verifies that a concrete <see cref="IModule"/> implementation without a public parameterless
    /// constructor is silently skipped during discovery rather than aborting the scan.
    /// </summary>
    [Fact]
    public void RegisterModules_WithModuleLackingPublicParameterlessCtor_SkipsItWithoutThrowing()
    {
        // Arrange
        TestModule.RegisterModuleCalled = false;
        var services = new ServiceCollection();

        // Act — the test assembly contains both TestModule (instantiable) and CtorlessModule (not).
        var act = () => services.RegisterModules(Assembly.GetExecutingAssembly());

        // Assert — CtorlessModule must not cause Activator.CreateInstance to throw.
        act.Should().NotThrow();
        var provider = services.BuildServiceProvider();
        var modules = provider.GetRequiredService<IReadOnlyList<IModule>>();
        modules.Should().ContainSingle(m => m is TestModule);
        modules.Should().NotContain(m => m is CtorlessModule);
    }

    /// <summary>
    /// Verifies that calling <c>RegisterModules</c> more than once accumulates the modules from
    /// every call, rather than silently dropping all but the last registration.
    /// </summary>
    [Fact]
    public void RegisterModules_CalledTwice_AccumulatesModulesFromBothCalls()
    {
        // Arrange
        var services = new ServiceCollection();
        var testAssembly = Assembly.GetExecutingAssembly();

        // Act — two registrations of the same assembly discover TestModule twice.
        services.RegisterModules(testAssembly);
        services.RegisterModules(testAssembly);

        // Assert — both calls' modules survive (a plain AddSingleton would expose only the last).
        var provider = services.BuildServiceProvider();
        var modules = provider.GetRequiredService<IReadOnlyList<IModule>>();
        modules.Count(m => m is TestModule).Should().Be(2);
    }

    /// <summary>
    /// Verifies that calling <c>MapEndpoints</c> without a prior <c>RegisterModules</c> call throws
    /// <see cref="InvalidOperationException"/> from <c>GetRequiredService</c> rather than silently
    /// mapping nothing.
    /// </summary>
    [Fact]
    public void MapEndpoints_WithoutPriorRegisterModules_ThrowsInvalidOperationException()
    {
        // Arrange — a builder whose DI container never had RegisterModules() called on it, so
        // IReadOnlyList<IModule> was never registered.
        var builder = WebApplication.CreateBuilder();
        var app = builder.Build();

        // Act
        Func<WebApplication> act = app.MapEndpoints;
