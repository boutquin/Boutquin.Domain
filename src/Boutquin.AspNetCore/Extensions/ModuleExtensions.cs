...
public static class ModuleExtensions
{
        ...
        params Assembly[] assemblies)
    {
        ...
        // Merge with any modules registered by a prior RegisterModules call so MapEndpoints sees
        // their union. A plain AddSingleton would append a second IReadOnlyList<IModule> descriptor,
        // and GetRequiredService resolves only the last registration — silently dropping the
        // earlier call's modules. We instead remove the prior registration and prepend its modules.
        var existing = services.FirstOrDefault(d => d.ServiceType == typeof(IReadOnlyList<IModule>));
        if (existing?.ImplementationInstance is IReadOnlyList<IModule> priorModules)
        {
            services.Remove(existing);
            registeredModules.InsertRange(0, priorModules);
        }
        ...
        // AsReadOnly() hands out a ReadOnlyCollection so a resolved module cannot down-cast the
        // instance to List<IModule> and mutate the shared registration.
        services.AddSingleton<IReadOnlyList<IModule>>(registeredModules.AsReadOnly());
    ...
    /// <see cref="IModule"/> <em>and expose a public parameterless constructor</em>, then creates an
    /// instance of each via that constructor. Concrete implementations without a public parameterless
    /// constructor are skipped rather than letting <see cref="Activator.CreateInstance(Type)"/> throw
    /// <see cref="MissingMethodException"/> and abort the whole scan. Assemblies whose types cannot be
    /// fully loaded (a <see cref="ReflectionTypeLoadException"/> from <c>GetTypes()</c>, common with
    /// optional/plugin assemblies) contribute only their successfully-loaded types rather than
    ...
    /// <summary>
    /// Returns the types defined in <paramref name="assembly"/>, tolerating partial load failures.
    /// </summary>
    /// <remarks>
    /// <see cref="Assembly.GetTypes"/> throws <see cref="ReflectionTypeLoadException"/> if <em>any</em>
    /// type fails to load (e.g. a missing transitive dependency in an optional assembly), discarding the
    /// types that <em>did</em> load. Catching it and returning <see cref="ReflectionTypeLoadException.Types"/>
    /// (filtered to non-null entries) keeps module discovery resilient to a single bad assembly.
    /// </remarks>
    /// <param name="assembly">The assembly to enumerate.</param>
    /// <returns>The loadable types in <paramref name="assembly"/>.</returns>
