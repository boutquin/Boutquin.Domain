...
public static class JsonElementExtensions
{
    ...
    /// Thrown when <paramref name="element"/> has <see cref="JsonValueKind.Undefined"/> — the value kind of
    /// a <c>default(JsonElement)</c> or an absent property accessed via <c>TryGetProperty</c>. Such an
    /// element has no JSON to write, so deserialization is undefined; this surfaces the misuse with a clear
    /// message instead of the opaque <see cref="InvalidOperationException"/> that <c>WriteTo</c> would throw.
    /// </exception>
    /// <example>
    /// <code>
    /// JsonElement element = ...;
    ...
    public static T? ToObject<T>(this JsonElement element, JsonSerializerOptions? options = null)
    {
        if (element.ValueKind == JsonValueKind.Undefined)
        {
            throw new ArgumentException(
                "Cannot deserialize a JsonElement with ValueKind 'Undefined' (e.g. default(JsonElement) or an absent property).",
                nameof(element));
        }
