    ...
    where TValue : notnull
{
    ...
    /// </summary>
    /// <returns>A string that represents the value of the strongly typed ID, formatted with
    /// <see cref="CultureInfo.InvariantCulture"/>.</returns>
    /// <remarks>
    /// Formatting is always invariant-culture, never the ambient <see cref="CultureInfo.CurrentCulture"/>:
    /// an identifier is a stable, machine-comparable token (used in logs, URLs, cache keys, equality
    /// checks against persisted values), not user-facing display text, so its rendering must not vary by
    /// thread culture — a culture-sensitive <typeparamref name="TValue"/> (e.g. a decimal-backed id)
    /// would otherwise render with a different decimal separator per locale.
    /// </remarks>
