...
/// else
/// {
///     Console.WriteLine($"Operation failed: {result.Error.Description}");
/// }
/// </code>
/// </example>
public class Result
{
    ...
    {
        // Guard every construction path (Failure, Failure<T>, and the implicit Error operators all
        // route here). A null error would slip past the `error == Error.None` check below — record
        // equality with a null operand is false — and be stored, surfacing as an NRE at access time.
    ...
    /// <exception cref="InvalidOperationException">Thrown when <paramref name="error"/> is
    /// <see cref="Error.None"/> — a failed result must carry a real error.</exception>
    ...
    /// Thrown when <paramref name="value"/> is <see langword="null"/>. A successful result must carry a
    /// value; constructing one from <see langword="null"/> would defeat the non-null guarantee on
    /// <see cref="Result{TValue}.Value"/> and hand a latent null-dereference to the caller. Use
    /// <see cref="Create{TValue}"/> when <see langword="null"/> should be treated as a failure instead.
    /// </exception>
    /// <example>
    /// <code>
    ...
    {
        // A success that carries null violates the [NotNull] contract on Result<TValue>.Value.
        // `value is null` is a no-op for non-nullable value types (the JIT elides it, so no boxing),
        // and rejects null reference / Nullable<T> values. Callers wanting null-as-failure use Create.
        if (value is null)
        {
            throw new ArgumentNullException(
                nameof(value),
                "A successful result cannot carry a null value. Use Result.Create for null-tolerant construction.");
        }
    ...
    /// <param name="description">The human-readable cancellation message. Defaults to "The request was cancelled.".</param>
    /// <returns>A failed result carrying an <see cref="ErrorType.Cancelled"/> (499) error.</returns>
    /// <remarks>
    /// Cancellation maps to the dedicated <see cref="ErrorType.Cancelled"/> (the de-facto 499
    /// "client closed request" code) rather than <see cref="ErrorType.RequestTimeout"/> (408), so a
    /// caller-initiated cancellation is not conflated with a server-side timeout when translated to a
    /// transport status.
    /// </remarks>
    public static Result Cancelled(string code = "Error.Cancellation", string description = "The request was cancelled.") =>
        Failure(Error.Cancelled(code, description));
