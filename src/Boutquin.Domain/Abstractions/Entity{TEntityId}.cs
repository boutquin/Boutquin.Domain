    ...
    where TEntityId : notnull
{
    ...
    /// <remarks>
    /// The identifier is accepted as-is and is not validated against <c>default(TEntityId)</c>. Passing
    /// <c>default</c> (e.g. <c>0</c>, <c>Guid.Empty</c>, or <c>null</c>) produces a <em>transient</em>
    /// entity — one that equals only itself by reference (see <see cref="Equals(Entity{TEntityId}?)"/>),
    /// never another entity, even one constructed with the same default id. This is intentional so that
    /// not-yet-persisted entities are never treated as equal; supply a real, non-default identity for
    /// value-based identity equality.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Returns an immutable <em>snapshot</em> (a copy), not a live view, so the caller can safely
    /// iterate it even if a handler raises new events or clears the buffer mid-dispatch. The entity is
    /// the sole authority on what events occurred — external code can only observe and clear them.
    /// </remarks>
    /// <returns>A read-only snapshot of the domain events raised so far, in chronological order.</returns>
    ...
    public bool Equals(Entity<TEntityId>? other)
    {
        ...
        // Different concrete entity types are never equal, even when they share the same TEntityId
        // and Id value (e.g. Order(42) must never equal Customer(42)). The generic base type alone
        // does not distinguish them, so compare the runtime types explicitly.
