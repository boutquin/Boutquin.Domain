...
/// </summary>
/// <param name="Code">A unique error code identifying the type of error (e.g. <c>"Error.NullValue"</c>).</param>
/// <param name="Description">The human-readable message describing the error (e.g. <c>"Null value was provided"</c>).
/// The identifier role is played by <paramref name="Code"/>.</param>
...
/// Each error is identified by a unique code and a human-readable message that describes the error.
/// This record is immutable, meaning that once an error is created, it cannot be modified.
/// </para>
/// <para>
/// <b>Equality contract:</b> because this is a positional <c>record</c>, value equality compares
/// <em>all three</em> components — <see cref="Code"/>, <see cref="Description"/>, and <see cref="ErrorType"/>.
/// Two errors with the same code and message but a different <see cref="ErrorType"/> are therefore
/// <em>not</em> equal, and an error with an empty code/message but a non-<see cref="ErrorType.None"/>
/// type is <em>not</em> equal to <see cref="None"/>. Consumers that need to identify an error by
/// <see cref="Code"/> alone should compare <see cref="Code"/> explicitly rather than the whole record.
/// </para>
/// <para>
...
/// </code>
/// </example>
public record Error(string Code, string Description, ErrorType ErrorType = default)
{
    /// <summary>
    /// Represents no error.
    /// </summary>
    ...
    public static Error RequestTimeout(string code, string description) => new(code, description, ErrorType.RequestTimeout);

    /// <summary>Creates a <see cref="ErrorType.Cancelled"/> (499) error representing a caller-initiated cancellation.</summary>
    public static Error Cancelled(string code, string description) => new(code, description, ErrorType.Cancelled);
