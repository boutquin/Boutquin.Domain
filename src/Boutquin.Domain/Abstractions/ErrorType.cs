...
/// error-to-HTTP-response mapping across application layers. The sole exception is
/// <see cref="Cancelled"/> (499), the de-facto "client closed request" code that has no
/// <see cref="HttpStatusCode"/> member.
...
public enum ErrorType
{
    ...
    /// <summary>
    /// Represents a caller-initiated cancellation. Maps to the de-facto 499 "client closed request"
    /// status code, which has no <see cref="HttpStatusCode"/> member. Distinct from
    /// <see cref="RequestTimeout"/> (408), which denotes a server-side timeout.
    /// </summary>
    Cancelled = 499,
