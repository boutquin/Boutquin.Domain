...
/// <remarks>
/// <para>
/// <b>Contract:</b> On .NET 10, <see cref="System.Text.Json"/> already supports <see cref="DateOnly"/>
/// dictionary keys. This factory adds explicit rejection of null dictionary values on read while
/// writing ISO 8601 keys and preserving the concrete dictionary type. Registering this factory alone
/// is sufficient; a separate <see cref="DateOnlyConverter"/> registration is optional.
/// </para>
/// <para>
...
public sealed class DateOnlyDictionaryConverterFactory : JsonConverterFactory
{
    ...
    /// <remarks>
    /// <para>
    /// <b>Why <see cref="Type.MakeGenericType"/> and <see cref="Activator.CreateInstance(Type, object?[])"/>:</b>
    ...
    private sealed class SortedDateOnlyDictionaryConverter<TValue> : JsonConverter<SortedDictionary<DateOnly, TValue>>
    {
        ...
        /// registered. Keys use the DateOnly converter's property-name reader; scalar Read overrides
        /// do not define dictionary-key parsing. Written keys always use ISO 8601.
        /// </remarks>
        ...
        /// <c>JsonSerializer.Deserialize</c> for the key expects a value token, but JSON object keys
        /// arrive as property names. The converter's <c>ReadAsPropertyName</c> handles that token
        ...
        {
            // STJ positions the reader at the opening token when invoking a converter. Assert it is an
            // object start so a non-object payload (array, bare value) fails with a clear JsonException
            // rather than silently yielding an empty dictionary on a pre-consumed reader.
            if (reader.TokenType != JsonTokenType.StartObject)
            {
                throw new JsonException($"Expected the start of an object for the dictionary but found {reader.TokenType}.");
            }
            ...
            while (reader.Read())
            {
                ...
                // Guard the forward-only read: after EndObject the only valid token is a property name.
                // Malformed JSON (e.g. an array or a bare value) would otherwise reach GetString() and
                // surface as an opaque InvalidOperationException instead of a clear JsonException.
                if (reader.TokenType != JsonTokenType.PropertyName)
                {
                    throw new JsonException($"Expected a property name for the DateOnly key but found {reader.TokenType}.");
                }

                var key = _dateOnlyConverter.ReadAsPropertyName(ref reader, typeof(DateOnly), options);
                reader.Read();
    ...
    private sealed class UnsortedDateOnlyDictionaryConverter<TValue> : JsonConverter<Dictionary<DateOnly, TValue>>
    {
        ...
        public override Dictionary<DateOnly, TValue> Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            // STJ positions the reader at the opening token when invoking a converter. Assert it is an
            // object start so a non-object payload (array, bare value) fails with a clear JsonException
            // rather than silently yielding an empty dictionary on a pre-consumed reader. Mirrors the
            // guard on SortedDateOnlyDictionaryConverter.Read.
            if (reader.TokenType != JsonTokenType.StartObject)
            {
                throw new JsonException($"Expected the start of an object for the dictionary but found {reader.TokenType}.");
            }
