...
In Domain-Driven Design, an **entity** is defined by its identity — not by the values of its attributes. Two `Customer` objects with the same `Id` represent the same real-world customer even if one has an updated address. Equality requires both the same concrete entity type and the same non-default `Id`.
...
Returns an immutable snapshot of the domain events buffered by this entity. External code can observe events but cannot add or remove them, and can safely iterate the snapshot if the entity changes during dispatch.

### `void ClearDomainEvents()`

Clears the buffered domain events. Take a snapshot first, then clear it before dispatching the snapshot to prevent re-entrancy and duplicate dispatch in nested persistence operations.
...
Identity-based equality. Returns `false` for different concrete entity types or when either entity has a default Id (transient entities must not match). `ReferenceEquals` is checked first so that a transient entity still equals itself.
...
Marker interface extending Boutquin.Domain's own `INotification`. The package has no MediatR dependency; applications choose how to dispatch collected events.
