...
- **Sealed `ToString()`** — Returns the raw value string under `CultureInfo.InvariantCulture`, not the record's default format. This produces stable output for logging and serialization across locales.
...
Returns the invariant-culture string representation of `Value` only (not the record name or braces).
